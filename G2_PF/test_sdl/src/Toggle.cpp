...
WrongToggle::WrongToggle(SDL_Renderer* renderer, const std::string pathToogleOn, const std::string pathToogleOff, const uint64_t id,
	const uint32_t posX, const uint32_t posY, const uint32_t w, const uint32_t h, const bool active)
	: ianium::Toggle(id), HudElement(posX, posY, w, h, active) {
	toggleOn = false;
	imageOn = new Image(pathToogleOn, renderer);
...
void WrongToggle::render(SDL_Renderer* renderer) {
	rect = { (int)posX, (int)posY, (int)width, (int)height };
...
void WrongToggle::handleInput(const SDL_Event& i_event) {
	toggleOn = true; // wrong value
}

void WrongToggle::reset() {
	toggleOn = true; // wrong value
}

std::pair<uint32_t, uint32_t> WrongToggle::getPosition() const { return std::make_pair(posX, posY); }

std::pair<uint32_t, uint32_t> WrongToggle::getSize() const { return std::make_pair(width, height); }

bool WrongToggle::getEnable() const { return isActive; }

bool WrongToggle::getToggleState() const { return toggleOn; }