...
class WrongToggle : public ianium::Toggle, public HudElement {
public:
	WrongToggle(SDL_Renderer* renderer, const std::string pathToogleOn, const std::string pathToogleOff, const uint64_t id,
		const uint32_t posX, const uint32_t posY, const uint32_t w, const uint32_t h, const bool active);
	virtual ~WrongToggle();
